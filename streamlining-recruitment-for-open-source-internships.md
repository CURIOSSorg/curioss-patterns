...
Reduce the administrative burden of intern recruitment by including structured screening steps in the application process and standardising applicant communications.
...
* An open, ‘frictionless’ application generates applications that are time-consuming to review and include poorly matched candidates.
* Skills and background requirements vary significantly across projects. A single generic application process may not surface the information needed to assess fit for individual projects.
* Internship programs need to be managed within fixed timelines, creating pressure to complete recruitment quickly.

## Solution

Design the recruitment process to filter out mismatched applications early and keep applicants informed without generating additional queries.
...
### Add Structured Application Criteria
...
* Including a small number of open questions relevant to the program or to specific projects.
* Requesting a GitHub profile or portfolio alongside standard application materials.

The importance of meeting these criteria should be clearly communicated in the application guidelines.
...
The initial screening pass removes obvious mismatches, such as graduate students applying to undergraduate-only roles, or applications that do not meet the minimum requirements stated in the project description.
...
Produce an FAQ document that covers key information about the program, timeline, and the standard turnaround for notifying applicants of the outcome.
...
We set a cap of 30 applications per position and monitor submissions actively as they arrive, removing obvious mismatches before the cap is reached. This keeps the review pool to a manageable size. 
...
Finally, although the interview process often ends in disappointment for the majority of applicants, we feel strongly that we can offer value to help candidates improve their resumes, applications and interview skills. We work hard to give feedback and here is a blog post we wrote and made available on our website as an example of some of the feedback we provide.
...
We use a scoring metric that balances the skills needed to perform the internship with providing opportunities for students who may not have a computer sciences background. 
...
We conduct joint interviews with mentors where this is helpful and are involved in the selection process throughout. 
...
We collect and curate all application materials centrally before distributing them to project mentors, which means mentors receive a prepared shortlist rather than an unmanaged pool of submissions. We step in as a second interviewer where a mentor would otherwise be conducting the process alone or is unavailable due to conference travel or other commitments and we provide notes to help mentors make their final decision.
...
We are working with WISCURDS (Wisconsin Undergraduate Research in Data Science), an undergraduate research focused experiential learning program housed within the Data Science Institute to explore ways to share the administrative load across both programs.
